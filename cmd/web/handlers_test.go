...
func TestShowSnippet(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid ID", "/snippet/1", http.StatusOK, []byte("An old Snippet")},
		{"Non-existent ID", "/snippet/2", http.StatusNotFound, nil},
		{"Negative ID", "/snippet/-1", http.StatusNotFound, nil},
		{"Decimal ID", "/snippet/1.12", http.StatusNotFound, nil},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if code != tt.wantCode {
				t.Errorf("want %v; got %v", tt.wantCode, code)
			}

			if !bytes.Contains(body, tt.wantBody) {
				t.Errorf("want %v; got %v", tt.wantBody, body)
			}
		})
	}
}
...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Duplicate email", "Bob", "mims@meeklife.net", "validPa$$word", csrfToken, http.StatusOK, []byte("Address is already in use")},
